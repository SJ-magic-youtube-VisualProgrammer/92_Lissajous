...
void FFT::update()
{
	...
	********************/
	...
	if(Gui_Global->b_Window) multiply_window(fft_x);
	...
	*/
	...
	cal_artSin_xy(artSin_x1, artSin_y1, 1);
	cal_artSin_xy(artSin_x2, artSin_y2, -1);
	
	fft(artSin_x1, artSin_y1, true);
	fft(artSin_x2, artSin_y2, true);
	
	if(Gui_Global->b_Window_artSin) { multiply_window(artSin_x1); multiply_window(artSin_x2); }
...
/******************************
******************************/
double FFT::get_Gain(int id) const
{
...
/******************************
******************************/
double FFT::get_Gain2(int id) const
{
...
/******************************
******************************/
double FFT::get_phase_rad(int id) const
{
	if(phase_rad_madeFromGain.size() <= id)	return 0;
	else									return phase_rad[id];
}
...
void FFT::cal_Gain_smoothed(vector <double>& _Gain_smoothed, double k)
{
	for(int i = 0; i < Gain.size(); i++){
		...
		// Gain_smoothed[i] = max( Gain_smoothed[i] * Gui_Global->k_smooth, Gain[i] );
	}
}
...
/******************************
******************************/
void FFT::cal_artSin_xy(vector <double>& artSin_x, vector <double>& artSin_y, int sgn)
{
	if(sgn < 0)	sgn = -1;
	else		sgn = 1;
	...
	for(int i = 1; i < artSin_x.size()/2; i++){
		if( (Gui_Global->ArtSin_Band_min <= i) && (i <= Gui_Global->ArtSin_Band_max) ){
...
/******************************
******************************/
void FFT::multiply_window(vector <double>& _x)
{
